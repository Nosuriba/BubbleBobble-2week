...
{
	int blockCnt = 0;
	int cnt = 0;								/// Ãﬁ€Ø∏ÇÃèÍèäÇ±∏æΩÇ∑ÇÈÇΩÇﬂÇÃÇ‡ÇÃ
	std::string name[] = { "0", "2" };			/// ì¡íËÇÃÉXÉeÅ[ÉWÉ`ÉbÉvÇì«Ç›çûÇﬁÇÊÇ§Ç…ÇµÇƒÇ¢ÇÈ(âºê›íË)
...
void GameManager::BubbleCollision(const Input& p)
{
	for (auto itr = bubbles.begin(); itr != bubbles.end(); ++itr)
	{
		...
		for (auto bubble = bubbles.begin(); bubble != bubbles.end(); ++bubble)
		{
			...
			/// ïÇÇ¢ÇƒÇ¢ÇÈñAìØémÇ™ìñÇΩÇ¡ÇΩéûÇ«Ç§ìÆÇ≠Ç©ÇÃèàóù
			if (itr != bubble)
			{
